...
def test_bigram_probabilities(analyzer):
    ...
    assert pytest.approx(analyzer.get_bigram_prob(('the', 'cat'))) == 2 / analyzer.word_occurrence['the']
...
def test_treeview_sorting(treeview):
    ...
    items = [(treeview.tree.set(item, 'Col2'), item) for item in treeview.tree.get_children('')]
    assert [float(val) for val, _ in items] == [5.0, 10.0, 15.0]
    
    # Test string sorting
    treeview.sort_by('Col1')
    items = [(treeview.tree.set(item, 'Col1'), item) for item in treeview.tree.get_children('')]
